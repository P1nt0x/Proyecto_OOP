...
try:
  ...
  if platform.system() == "Linux":  
    from PIL import Image, ImageTk
      ...
      mssg.showerror("Error.", f"Error {type(e)}: {e}")
...
class Inventario:  
  def __init__(self, master=None):
    try:
      ...
      self.db_name = self.path + r'/db/Inventario.db'
      ...
      if platform.system() == "Windows":
        self.win.iconbitmap(self.path + r'/img/f2.ico')
      elif platform.system() == "Linux":
        ima = Image.open('/img/nino-modified.png') #Esta foto esta para mostrarse solamente en sistemas linux
...
# Función para validar fecha
  ...
  def updateProvider(self, event = None):
    ...
    try:
      ...
      if r != None:
        ...
        if r[1] != None: self.razonSocial.insert(0,r[1])
        self.ciudad.delete(0,"end")
        if r[2] != None: self.ciudad.insert(0,r[2])
      else:
        self.razonSocial.delete(0,"end")
        self.ciudad.delete(0,"end")
    except Exception as e:
  ...
  def updateProduct(self, event = None):
    ...
    try:
      ...
      if r != None:
        ...
        if r[2] != None: self.descripcion.insert(0,r[2])
        self.unidad.delete(0,"end")
        if r[3] != None: self.unidad.insert(0,r[3])
        self.cantidad.delete(0,"end")
        if r[4] != None: self.cantidad.insert(0,r[4])
        self.precio.delete(0,"end")
        if r[5] != None: self.precio.insert(0,r[5])
        self.fecha.delete(0,"end")
        if r[6] != None: self.fecha.insert(0,r[6])
  ...
  def run_Query(self, query, parametros = ()):
    ...
    try:
      with sqlite3.connect(self.db_name) as conn:
          ...
          parametros2 = []
          for p in parametros:
            if p == "": parametros2.append(None)
            else: parametros2.append(p)
  ...
  def lee_treeProductos(self):
    ...
    try:      
      self.run_Query("CREATE TABLE IF NOT EXISTS Proveedor(idNitProv varchar(15) primary key not null,Razon_Social varchar(50),Ciudad varchar(50))")
      self.run_Query("CREATE TABLE IF NOT EXISTS Inventario(IdNit VarChar(15), Codigo VarChar(10), Descripcion VarChar(15), "
       + "Und VarChar(10), Cantidad INTEGER, Precio FLOAT, Fecha DATE, PRIMARY KEY(IdNit, Codigo), "
